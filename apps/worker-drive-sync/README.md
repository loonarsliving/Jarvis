...
Build/run: `pnpm --filter worker-drive-sync build && pnpm --filter worker-drive-sync start`
(or `pnpm --filter worker-drive-sync dev` for local iteration via `tsx`).
